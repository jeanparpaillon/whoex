defmodule Whoex.Builder do
  ...
  `Whoex.Builder` also imports the `Whoex.Conn` module, making functions like
  `send_resp/3` available.

  ## Options

  When used, the following options are accepted by `Whoex.Builder`:
  ...
  Internally, `Whoex.Builder` implements the `Whoex.Plug` behaviour, which means both
  ...
  The `call/2` function that `Whoex.Builder` provides is used internally to
  ...
  plug chain:
      ...
      defmodule PlugWithCustomCall do
        ...
        def call(conn, opts) do
          conn
          |> super(opts) # calls Whoex.Logger
        end
      end

  ## Halting a plug pipeline

  A plug pipeline can be halted with `Whoex.Conn.halt/1`. The builder will
  ...
  defmacro __using__(opts) do
    quote do
      ...
      import Whoex.Conn
      import Whoex.Builder, only: [plug: 1, plug: 2, builder_opts: 0]

      Module.register_attribute(__MODULE__, :plugs, accumulate: true)
      @before_compile Whoex.Builder
    end
  end
  ...
  defmacro __before_compile__(env) do
    ...
    {conn, body} = Whoex.Builder.compile(env, plugs, builder_opts)
  ...
  defmacro builder_opts() do
    quote do
      Whoex.Builder.__builder_opts__(__MODULE__)
    end
  end
  ...
  ## Examples
      ...
      Whoex.Builder.compile(env, [
        {Whoex.Logger, [], true}, # no guards, as added by the Whoex.Builder.plug/2 macro
        {Whoex.Head, [], quote(do: a when is_binary(a))}
      ], [])

  """
  @spec compile(Macro.Env.t(), [{plug, Whoex.Plug.opts(), Macro.t()}], Keyword.t()) ::
          {Macro.t(), Macro.t()}
  ...
  defp log_halt(plug_type, plug, env, builder_opts) do
    if level = builder_opts[:log_on_halt] do
      ...
      quote do
        require Logger
        # Matching, to make Dialyzer happy on code executing Whoex.Builder.compile/3
